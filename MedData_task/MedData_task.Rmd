...
cat(paste0("Стоит обратить внимание на то, что из ", nrow(data_clean), " пациентов у ",
          ...
          signif(sd(data_clean$BMI), 3), ", а от ожирения стадало ",
...
```{r roc1, fig.height=4, fig.width=4, message=FALSE}
...
Форма ROC-кривой указывает на предсказательную способность уровня гемоглобина для летального исхода в течение 24 часов. Площадь под кривой (AUC) составляет **`r signif(roc_Death_Hb$ci[2], 3)`** (95% ДИ: *`r signif(roc_Death_Hb$ci[1], 3)` - `r signif(roc_Death_Hb$ci[3], 3)`*), что значительно выше значения 0.5, характерного для случайной классификации. Такое значение AUC может быть объяснено тем, что уровни гемоглобина значимо различаются у пациентов с летальным исходом и без него, что было подтверждено результатами описательной статистики.
