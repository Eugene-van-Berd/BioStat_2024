...
-   Добавьте на график линию тренда. Удалите доверительную область и сделайте линию прямой.
...
-   Постарайтесь избежать коллизий в легенде, при этом сохранив и цветовую 
    палитру для раскраски точек по факультетам.
...
```{r col1, fig.height=15, fig.width=15}
...
```{r col2, fig.height=15, fig.width=15}
...
```{r col3, fig.height=15, fig.width=15, message=FALSE, warning=FALSE}
...
-   Изучите функцию coord_flip(). Как вы думаете, когда она может быть полезна? Как вы думаете, в чем
    ее плюсы и минусы?

Данная функция позволяет поменять местами оси X и Y.

Функция может быть полезна, когда на графике много категорий, и горизонтальное представление может улучшить читабельность. Кроме того, coord_flip() позволяет избежать наложения текста, если названия категорий длинные.

Однако функция coord_flip() считается устаревшей, так как для упрощения кода можно просто поменять местами x и y в функции aes(). Кроме того, горизонтальные графики могут быть менее привычны для пользователей, что может затруднить интерпретацию данных.
...
```{r ggarrange, fig.height=12, fig.width=15}
...
-   Измените порядок ваших фигур на первом графике слева направо следующим образом: маглорожденные, чистокровные, полукровки. Скорректируйте название оси. 
...
```{r Potions1, fig.height=15, fig.width=20}
...
Potions1 <- data %>% 
  ...
  ggplot(aes(x = bloodStatus, y = meanPotionsExam, 
             ymin = lower_Bound, ymax = upper_Bound, colour = bloodStatus))+
...
Возможно дело в самом экзамене? Где-то хороши чистокровки, а где-то грязнокровки!

```{r Potions2, fig.height=15, fig.width=20, message=FALSE, warning=FALSE}
...
```
Нет! Средний бал всегда находится в диапазоне 45-50 баллов, но только не для Грязнокровок на Зельеварении. Но всех ли грязнокровок не любит профессор зельеварения?

```{r Potions3, fig.height=15, fig.width=20, message=FALSE, warning=FALSE}
...
```
Если сравнивать медианные значения, то по каждому из экзаменов девушки набирают больше установелнного выше диапазона: 45-50 баллов, а перани ниже! И снова это все не касается зельеварения! Профессор зельеварения еще и сексист?

```{r Potions4, fig.height=15, fig.width=20, message=FALSE, warning=FALSE}
...
```
О да! Он сексист и любитель чистокровок, все сходится!
А теперь все объединим!

```{r Potions5, fig.height=30, fig.width=60, message=FALSE, warning=FALSE}
...
Распределение баллов на факультетах: Скрипичные графики показывают разное распределение баллов на факультетах. Например, если у одного факультета график широкий в центре, это говорит о том, что у большинства студентов баллы сгруппированы вокруг среднего значения.
Средние значения и медианы: Если красная точка (среднее значение) находится далеко от линии медианы (середина boxplot), это может говорить о смещении распределения или наличии выбросов.
Сравнение между полами: Фасеты по полу позволяют видеть, есть ли различия в распределении баллов между мальчиками и девочками на каждом факультете. Например, может быть видно, что средний балл выше для мальчиков на одном факультете, но ниже на другом.
Линия на уровне 0: Горизонтальная линия на уровне 0 может служить отправной точкой для сравнения, показывая, что баллы могут быть как положительными, так и отрицательными.
Дополнительные наблюдения:
Слизерин: Если распределение баллов у факультета сильно вытянуто или асимметрично, это может говорить о том, что есть несколько студентов с очень высокими или низкими баллами.
Гриффиндор: Широкий график указывает на то, что разброс баллов студентов этого факультета более равномерный.
Таким образом, этот график позволяет глубже понять, как распределяются баллы на факультетах, есть ли различия между полами и насколько стабильно распределение внутри факультетов.
